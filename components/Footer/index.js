...
function Footer() {
  ...
  //     if ((footerRect.top + footerRect.height >= 0) && (footerRect.top <= windowHeight)) {
  //       setVisible(true);
  //       // console.log('its working')
  //     }
  //   }
  //   // console.log("visible : ", visible, "mounted: ", mounted);
  // })
    ...
    <footer className={`w-full shadow bg-primarycolor text-slate-200 py-5 xs:mb-[2rem] md:mb-0`} ref={footerRef}>
      <div className='w-11/12 mx-auto flex md:flex-row xs:flex-col justify-between place-items-center gap-5'>
        <div className="flex mt-4 space-x-6 sm:justify-center sm:mt-0">
          <Link href="https://www.facebook.com/profile.php?id=100095223584273&mibextid=LQQJ4d" target='blank' className="text-slate-500 hover:text-slate-200">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" /></svg>
            <span className="sr-only">Facebook page</span>
          </Link>
          <Link href="https://instagram.com/jnualumniassociation?igshid=MzRlODBiNWFlZA==" target='blank' className="text-slate-500 hover:text-slate-200">
        ...
        <div className='flex sm:flex-row xs:flex-col justify-center items-center gap-5'>
          <p className='text-slate-400 font-light tracking-wider xs:text-center'>© 2023  JNU Alumni Association, All Rights Reserved</p>
        </div>
        <ul className='flex xs:flex-col sm:flex-row justify-center place-items-center sm:gap-10 xs:gap-5'>
          ...
          <li>
            <Link href="/join" className=' text-slate-500 hover:text-slate-200 font-light'>Join</Link>
          </li>
        </ul>
      </div>
    </footer>
