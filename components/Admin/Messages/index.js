  ...
  return (
    <div className='min-h-screen w-full flex flex-col justify-start items-start text-black'>
      <div className='sticky top-2 my-2 mx-2'>
        <input className='w-full max-w-[300px] outline-none p-2 border-[1px] border-primarycolor rounded-md tracking-wide font-medium' placeholder='Search Messages' onChange={(e) => setSearch(e.target.value)} />
      </div>
      {
        messages.length === 0 ? <p className='text-3xl text-center font-medium'>No Messages</p> : (search !== '' ? messages
